...
- `make install` honours a `GOBIN` from the environment or `go env -w`, so it and
  `go install github.com/WariKoda/drift@latest` no longer install two binaries that
  shadow each other in `$PATH`. Without a `GOBIN` the target stays `~/.local/bin`
